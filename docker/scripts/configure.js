...
//
// Based on coollabsio/openclaw configure.js with Cloudflare Tunnel extensions:
//   - OPENCLAW_STRIP_PROXY_HEADERS → sets trustedProxies + disableDeviceAuth
//   - OPENCLAW_ALLOWED_ORIGINS → sets controlUi.allowedOrigins
//
...
// Gateway token: required via OPENCLAW_GATEWAY_TOKEN env var
...
// ══════════════════════════════════════════════════════════════════════════════
if (process.env.OPENCLAW_STRIP_PROXY_HEADERS === "true") {
  console.log("[configure] ⚡ Cloudflare Tunnel mode: configuring gateway for proxy trust");

  // Trust loopback (nginx → gateway is 127.0.0.1)
  config.gateway.trustedProxies = ["127.0.0.1"];

  // Allow insecure auth (no device pairing needed via tunnel)
  ensure(config, "gateway", "controlUi");
  config.gateway.controlUi.allowInsecureAuth = true;
